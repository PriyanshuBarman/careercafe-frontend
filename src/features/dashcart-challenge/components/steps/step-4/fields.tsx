...
type Step4FieldsProps = {
  control: Control<ChallengeFormData>;
  setValue: UseFormSetValue<ChallengeFormData>;
};

export default function Step4Fields({ control, setValue }: Step4FieldsProps) {
  return (
...
function LockedPlanCard({ control }: { control: Control<ChallengeFormData> }) {
  const [entryModel, strategicInitiatives, hardestUnfundedInitiative] =
    useWatch({
      control,
      name: ["entryModel", "strategicInitiatives", "hardestUnfundedInitiative"],
    });
  ...
  const planSummaryItems = [
    ...
    { label: "Spend", value: planSummary ? `₹${planSummary.totalCost}L` : "—" },
    {
      label: "Service",
